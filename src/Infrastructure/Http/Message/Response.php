...
class Response
{
    ...
    public function sendHeaders(): void
    {
        ...
        header(
            sprintf('HTTP/1.0 %s %s', $this->status, self::$statusTexts[$this->status] ?? 'unknown status'),
            true,
            $this->status,
        );
    }

    public function addHeader(string $key, int|float|bool|string|\Stringable $value): self
    {
